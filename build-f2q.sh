...
if [ -n "$TG_BOT_TOKEN" ] && [ -n "$TG_CHAT_ID" ]; then
    tg_post_msg "<b>🔨 Build Started</b>%0A<b>Device:</b> f2q%0A<b>Kernel Version:</b> ${KERNEL_VERSION}%0A<b>Compiler:</b> Clang r522817"
fi
...
if [ -n "$TG_BOT_TOKEN" ] && [ -n "$TG_CHAT_ID" ]; then
    ...
    curl -s -F document=@"$ZIPNAME" "https://api.telegram.org/bot${TG_BOT_TOKEN}/sendDocument" \
        ...
        -F parse_mode="HTML" \
        -F caption="<b>✅ Build Finished</b>%0A<b>Device:</b> f2q%0A<b>Kernel Version:</b> ${KERNEL_VERSION}%0A<b>Time:</b> $((SECONDS / 60))m $((SECONDS % 60))s%0A<b>Zip:</b> $ZIPNAME%0A%0A<b>Changelog:</b>%0A<code>${CHANGELOG}</code>"
fi
